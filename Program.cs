...
namespace XOSC
{
    ...
    public static class HardwareService
    {
        ...
        public static (string Load, string Vram, string Temp) GetGpuStats(string gUnit, string vUnit, string tUnit)
        {
            ...
            try {
                ...
                string vr = vUnit == "GB" ? $"{Math.Round(used / 1073741824.0, 1)}/{Math.Round(total / 1073741824.0, 1)}GB" : $"{(used * 100 / total)}%";
    ...
    public static class Updater
    {
        ...
        private const string CommitApiUrl = "https://api.github.com/repos/hollyntt/XOSC/commits/master";

        public static async Task CheckForUpdates()
        {
            Status = "checking GitHub commits...";
            try {
                using var http = new HttpClient();
                http.DefaultRequestHeaders.Add("User-Agent", "XOSC-Updater");
                var resp = await http.GetStringAsync(CommitApiUrl);
                using var doc = JsonDocument.Parse(resp);
                string latestSha = doc.RootElement.GetProperty("sha").GetString()?.Substring(0, 7) ?? "";

                if (Program.Config.Version.Contains(latestSha)) { Status = "already up to date"; return; }

                Status = "fetching zip...";
                var zipData = await http.GetByteArrayAsync(Program.Config.PublishPath);
                ...
                if (entry != null) {
                    ...
                    Status = $"Update Found: {latestSha}"; NewVersionFound = true;
                }
    ...
    public static class MusicChatEngine
    {
        ...
        private static async Task Update()
        {
            ...
            if (DateTime.Now < _manualE) { SendOsc("/chatbox/input", $"💬 {_manualM}"); return; }
            ...
            if (cfg.PcMode) {
                ...
                if (mem.Count > 0) lns.Add(string.Join(" | ", mem));
            }
            ...
            SendOsc("/chatbox/input", output); _lastS = DateTime.Now; PacketsSent++;
        }
    ...
    class Program
    {
        ...
        static void DrawUI()
        {
            ...
            ImGui.Dummy(new Vector2(0, 20)); ImGui.SetCursorPosX(20); ImGui.TextColored(ColAccent, "XOSC"); ImGui.SetCursorPosX(20); ImGui.TextColored(new Vector4(0.5f, 0.5f, 0.6f, 1f), $"v{Config.Version}"); ImGui.Dummy(new Vector2(0, 20));
            ...
            ImGui.SetCursorPosY(sh - 50); ImGui.SetCursorPosX(20); ImGui.TextColored(Config.ChatboxEnabled ? new Vector4(0.35f, 0.95f, 0.55f, 1f) : new Vector4(1f, 0.33f, 0.33f, 1f), Config.ChatboxEnabled ? "● Live" : "● Paused");
            ...
            switch (_navPage) {
                case 0: Card("Dashboard", () => { ImGui.Text($"Packets: {MusicChatEngine.PacketsSent}"); ImGui.InputText("Override##field", ref _chatIn, 128); if (ImGui.Button("Send")) { MusicChatEngine.SetManual(_chatIn); _chatIn = ""; } }); break;
                ...
                case 5: Card("Updater", () => { ImGui.Checkbox("Beta Opt-in", ref Config.BetaOptIn); if (ImGui.Button("Check for Update")) Task.Run(() => Updater.CheckForUpdates()); if (Updater.NewVersionFound && ImGui.Button("Apply Update")) Updater.ApplyUpdate(); ImGui.Text($"Status: {Updater.Status}"); }); break;
            }
        ...
        public static void SaveConfig() { try { var opts = new JsonSerializerOptions { WriteIndented = true, IncludeFields = true }; File.WriteAllText(_path, JsonSerializer.Serialize(Config, opts)); } catch { } }
        static void LoadConfig() { if (!File.Exists(_path)) return; try { var opts = new JsonSerializerOptions { IncludeFields = true }; var loaded = JsonSerializer.Deserialize<AppConfig>(File.ReadAllText(_path), opts); if (loaded != null) Config = loaded; } catch { } }
    }
}